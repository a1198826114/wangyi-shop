...
const actions = {
  ...
  async getSearch({ commit }) {
    ...
    if (result.data.code === 200) {
      commit("RECIVE_SEARCH_LIST", result.data.data);
    }
  ...
  async getBanner({ commit }) {
    ...
    // console.log(result)
    if (result.data.code === 200) {
      commit("RECEIVE_BANNERS_LIST", result.data.banners);
    }
  ...
  async getRmdComd({ commit }) {
    ...
    console.log(result);
    if (result.data.code === 200) {
      commit("RECEIVE_RMDCOMD_LIST", result.data.data);
    }
    // console.log(result.data)
  },
  //获取热门商品的数据
  async getHotComd({ commit }) {
    ...
    // console.log(result)
    if (result.data.code === 200) {
      commit("RECEIVE_HOTCOMD_LIST", result.data.data);
    }
    // console.log(result.data)
  },
};
